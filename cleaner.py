...
def check_and_print_pdb(count, residue_buffer, residue_letter):
  ...
  if hasCA and hasN and hasC:
    ...
    chain = line[21]
    try:
      fastaseq[chain] = ''.join([fastaseq[chain],residue_letter]) # before was ' = "".join(init,residue_letter)'
    except KeyError:
...
#print(filename_stem, "".join(chainid), nres, flag_altpos, flag_insres, flag_modres, flag_misdns, flag_successful)
if nres > 0:
