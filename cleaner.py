...
def check_and_print_pdb(count, residue_buffer, residue_letter):
	...
	if hasCA and hasN and hasC:
		for line in residue_buffer:
			newnum = f'{count}'
			line_edit = f'{line[0:22]}{newnum:^6}{line[27:]}'
			pdbfile = pdbfile + line_edit

		chain = line[21]
		try:
			fastaseq[chain] = ''.join([fastaseq[chain],residue_letter]) 
		except KeyError:
...
### Program Start ###
parser = OptionParser(usage='%prog [options] <pdb> <chain id> <output>')
parser.add_option("--fastaout", action="store_true",help="Outputs a fasta file",default=False)
options, args = parser.parse_args()

if len(args) != 3:
  parse.error('Must specify both the pdb and the chain id as well as output directory')
...
if nres > 0:
	outfile = f'{outputdir}{filename_stem}_{chainid}.pdb'
