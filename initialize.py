...
fpath = f'{initial}{pdb}_out/{pdb}_out.pdb'
pocket = [line for line in open(f'{fpath}').readlines() \
...
log = open(f'{initial}surf.log','w')
for i,c in enumerate(glob.glob(f'{initial}*conf*')):
	# progress reporting
	if ((i+1)%100 == 0):
		print(f'{i+1}/{NConf}')

	attributes=os.path.basename(c)
	conf=attributes.split('_')[1]
	rotation=attributes.split('_')[2]
	tilt=attributes.split('_')[3]
	tran=attributes.split('_')[4]
	flip=attributes.split('_')[5]
	args=(surf,'-surfProbeGen',c,f'{initial}{pdb}.{conf}.{rotation}.{tilt}.{tran}.{flip}.SURF',3,.5)
	str_args = [ str(x) for x in args ]
	subprocess.run(str_args, stdout=log)
