...
parser.add_argument('-c', '--cutoff', dest='cutoff', default=1.50,
                metavar='C', help='Percentage cutoff for maximum number of alpha spheres in fpocket, \
                                                default behavior is 1.50 of target structure')
...
if checkpoint:
    ...
    identifier = ''.join([choice(ascii_letters + digits) for _ in range(10)])
    chkpt = f'{outputdir}{identifier}.chk'
    with open(chkpt, 'w') as chk:
        chk.write('0\n')
        for structure in tracker:
            chk.write(f'{structure}\n')
